...
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  if(afl->queue_cur && !(afl->queue_cur->ancestor_seed == NULL || afl->from_splicing) && (afl->read_flag && afl->write_flag && afl->mini_mode == 0) && virgin_map == afl->virgin_bits)
  ...
  while (i--) {
    ...
    if (unlikely(*current)) discover_word(afl, tmp_edge, &ret, current, virgin, virgin_local);
  ...
  if(afl->read_flag && afl->write_flag && afl->mini_mode == 0) afl->read_flag = 0;
  return ret;
}
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (unlikely(afl->schedule >= FAST && afl->schedule <= RARE) || afl->history_mode) {
    ...
    afl->cur_n_fuzz_idx = cksum % N_FUZZ_SIZE;
