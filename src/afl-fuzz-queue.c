...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  if(afl->use_splice_mutator || afl->from_splicing) q->splice = 1;
  else if(afl->queue_cur && afl->queue_cur->splice){
    q->splice = 1;
  }else q->splice = 0;
  if(afl->queue_cur != NULL && q->splice == 0){
  ...
  if(q->ancestor_seed){
    q->map = (u64 *)malloc(sizeof(u64) * afl->queue_cur->len);
    memcpy(q->map, afl->queue_cur->map, afl->queue_cur->len * sizeof(u64));
  }
  else if(q->splice){
    q->map = NULL;
  }
  ...
  q->related_bytes = NULL;
  q->FTI_analyzed = 0;
  q->related_num = 0;

  q->exec_history = 1;
  q->found_by_history = 1;
  q->exec_new = 1;
  q->found_by_new = 1;
  q->exec_random = 1;
  q->found_by_random = 1;

  q->useful_common_features = NULL;
  q->useful_common_features_idx = 0;

  q->feature_vec = M_Zeros(afl->centers_num,1);
  q->vec_update_time = 0;
  q->mutated_pos_num = -1;
  q->mutated_pos = NULL;
  ...
  if(q->ancestor_seed && q->initial_seed == 0){
    if(q->len > afl->queue_cur->len && afl->use_splice_mutator == 0){      
      q->initial_seed = 0;
      q->ancestor_seed = NULL;
      q->splice = 1;
    }
