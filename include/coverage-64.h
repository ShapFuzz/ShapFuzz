 ...
 * seen in ret. */
inline void discover_word(afl_state_t *afl, u32 tmp_edge, u8 *ret, u64 *current, u64 *virgin, u64 *virgin_local) {
  ...
  if(virgin_local){
    if(*current & *virgin_local){
      ...
      if(cur[0] && vir[0] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){ 
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 0;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[0];
          afl->new_edges_found_idx++;
        }
      }

      if(cur[1] && vir[1] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){ 
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 1;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[1];
          afl->new_edges_found_idx++;
        }
      } 
      ...
      if(cur[2] && vir[2] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 2;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[2];
          afl->new_edges_found_idx++;
        }
      } 
      if(cur[3] && vir[3] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 3;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[3];
          afl->new_edges_found_idx++;
        }
      } 
      if(cur[4] && vir[4] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 4;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[4];
          afl->new_edges_found_idx++;
        }
      } 
      if(cur[5] && vir[5] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 5;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[5];
          afl->new_edges_found_idx++;
        }
      } 
      if(cur[6] && vir[6] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 6;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[6];
          afl->new_edges_found_idx++;
        }
      } 
      if(cur[7] && vir[7] == 0xff){
        if(afl->read_flag && afl->write_flag && afl->mini_mode == 0){
          afl->new_edges_found[afl->new_edges_found_idx] = tmp_edge + 7;
          afl->new_cmps_state[afl->new_edges_found_idx] = cur[7];
          afl->new_edges_found_idx++;
        }
      }
