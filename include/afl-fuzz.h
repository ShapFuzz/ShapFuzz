...
kmeans_t* kmeans_new ( float **dataset, const int dataset_size, const int dataset_dim, const int K );
kmeans_t* kmeans_auto ( float **dataset, int dataset_size, int dataset_dim );
...
struct queue_entry {
  ...
  u32 exec_history;
  u32 found_by_history;
  u32 exec_new;
  u32 found_by_new;
  u32 exec_random;
  u32 found_by_random;

  u32* useful_common_features;
  u32 useful_common_features_idx;
  Matrix * feature_vec;
  u32 vec_update_time;
  u32 found_edges;
  u8 incre;
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u8 use_splice_mutator;
  u8 from_splicing;
  u64 new_edges_found[65537];
  u64 new_edges_found_idx;
  u8 read_flag;
  u8 write_flag;
  u8 mini_mode;
  u8 time_to_use_length_mutator;
  u32* cur_mutation_sequence;
  u32 cur_mutation_sequence_idx;
  u32* history_mutation_sequence;
  u32 history_mutation_sequence_idx;
  u32* new_mutation_sequence;
  u32 new_mutation_sequence_idx;
  u32 max_len;
  u8* cmp_states;
  u64 new_cmps_found[65537]; 
  u8 new_cmps_state[65537];
  u64 new_cmps_found_idx;

  u32 splice_stack[1024][2000]; 
  u32 splice_num;
  ...
  u32 exec_history_all;
  u32 found_by_history_all;
  u32 exec_new_all;
  u32 found_by_new_all;
  u32 exec_random_all;
  u32 found_by_random_all;
  u32 exec_all;
  u32 found_all;

  double* distribution;
  double distribution_sum;
  u32 distribution_analyzed;

  float **centers;
  u32 centers_num;

  u8 exist_centers;
  u64 centers_gen_time;
  u32 last_centers_realted_seeds;

  u32* feature_map;
  u32 num_edge;

  double* dataset_reward;
  double* hit_nums;
  u32 dataset_size;
  u32* tmp_mutated_pos;
  u8* tmp_mutated_pos_flag;
  u32 tmp_mutated_pos_idx;
  u64 cur_n_fuzz_idx;
  u8 record_flag;

  u64 last_show_time;
  u32 before;
  u64 family_record_time;
} afl_state_t;
...
#ifdef WORD_SIZE_64
void discover_word(afl_state_t *, u32 tmp_edge, u8 *ret, u64 *current, u64 *virgin, u64 *virgin_local);
#else
...
void update_cmps_state(afl_state_t *, void *, u32);
u8 FTI(afl_state_t *, u8 *, u8 *, u32);
